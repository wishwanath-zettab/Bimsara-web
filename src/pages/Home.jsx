...
import fbIcon from '../assets/images/fb-icon-2.jpg'
  ...
  return (
    <div className="bg-lines">
      ...
      <section className="relative overflow-hidden bg-lines lg:-mr-[100px]">
        {/* Logo */}
        <div className="hidden lg:block relative z-10 pt-6 lg:pt-[35px] pl-5 lg:pl-[8vw]">
        ...
        {/* Headline text */}
        <div className="relative z-10 px-5 lg:pl-[10vw] mt-[8px] lg:mt-[33px] pb-[40px] lg:pb-[100px] text-center lg:text-left">
          <h1 className="font-poppins hero-headline">
            <span className="block text-crimson text-[29px] md:text-[42px] lg:text-[42px] font-normal">Redefining</span>
            <span className="block text-crimson text-[29px] md:text-[42px] lg:text-[42px] font-semibold">Real Estate Brokering</span>
            <span className="block text-crimson text-[29px] md:text-[42px] lg:text-[42px] font-normal">since a quindecennial.</span>
          </h1>
        </div>
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                >
                  ...
                  <div className="dropdown-combobox-icon">
                    <svg className="w-[15px] h-[15px] text-crimson" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      ═══════════════════════════════════════════ */}
      <section className="mt-[20px] pt-[20px] lg:pt-[40px] pb-[60px] lg:pb-[80px] px-5 md:px-10 lg:px-[8vw]">
        {/* Three value props — plain crimson text per Figma */}
        <div className="flex flex-col sm:flex-row justify-center gap-2 lg:gap-0 pt-0">
          {['Trusted Advisor', 'Skilled Negotiator', 'Expert Facilitator'].map((title) => (
            <div key={title} className="text-center w-full sm:w-1/3">
              <span className="font-lato text-[24px] lg:text-[30px] font-semibold italic text-crimson tracking-[0.05em]">{title}</span>
            </div>
          ))}
        </div>
      ...
      ═══════════════════════════════════════════ */}
      <section className="bg-ebony py-12 lg:py-[96px] px-5 md:px-10 lg:px-[8vw] flex flex-col items-center lg:-mr-[100px]">
        <h2 className="font-lato text-[22px] lg:text-[40px] font-bold text-white mb-8 lg:mb-10 leading-tight">
          Our Services are for...
        </h2>
        <div className="flex flex-col items-center gap-[40px] mt-[40px] w-full max-w-[700px]">
          {[
            ...
            <Link key={item.label} to={item.path} className="dark-service-card group cursor-pointer">
              <div className="dark-service-card-inner">
                ...
                <img src={arrowRight2} alt="" className="h-[18px] w-auto group-hover:translate-x-1 transition-transform" loading="lazy" />
              </div>
            </Link>
          ))}
        </div>
      ...
      <section id="testimonials" className="pt-[53px] pb-12 lg:pt-[106px] lg:pb-[96px] px-5 md:px-10 lg:px-[8vw]">
        ...
        <div className="glass-card rounded-[20px] lg:rounded-[30px] p-6 lg:p-[60px] mb-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
            {testimonials.map((t) => (
              <div key={t.name} className="bg-white/80 rounded-[20px] p-[40px] backdrop-blur-sm flex flex-col">
                <h4 className="font-lato text-[20px] font-black text-royal-blue mb-2">{t.name}</h4>
                <p className="font-lato text-[18px] font-light text-ebony-clay leading-[28px] text-justify mt-[20px] mb-3">{t.quote}</p>
                <p className="font-lato text-[11px] font-light text-ebony-clay leading-[20px] text-justify mt-[1vh]">{t.role}</p>
                <p className="font-lato text-[11px] font-light text-ebony-clay leading-[20px] text-justify">{t.sub}</p>
                <div className="mt-auto pt-[5vh] flex justify-center">
                  <a href={t.video} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-[10px] cursor-pointer font-lato text-[18px] font-normal text-crimson border border-crimson rounded-[40px] px-[41px] py-[12px] w-max hover:bg-crimson hover:text-white transition-colors group">
      ...
      {/* ═══════════════════════════════════════════
          SECTION 5 — CONTACT
      ═══════════════════════════════════════════ */}
      <section id="contact" className="py-12 lg:py-[96px] px-5 md:px-10 lg:px-[8vw]">
        <h2 className="font-lato text-[24px] lg:text-[45px] font-normal text-crimson text-center mb-3">
          Contact Us
        </h2>
        <p className="font-lato text-[21px] lg:text-[22px] font-light text-ebony-clay leading-[28px] lg:leading-[32px] text-center mb-16 lg:mb-24 max-w-[967px] mx-auto">
          Get in touch with us for all your real estate requirements.<br /> We believe
          in building strong relationships that go long term with our clients.
        </p>

        {/* Directory (left) + Form (right) */}
        {(() => {
          const general = data.contacts.find((c) => c.category === 'GENERAL INQUIRIES')
          const rest    = data.contacts.filter((c) => c.category !== 'GENERAL INQUIRIES')
          const entry = (c, phoneBold = false) => (
            <div key={c.category} className="text-center">
              <p className="font-lato text-[16px] lg:text-[18px] font-bold text-royal-blue tracking-[0.05em] mb-2">
                {c.category}
              </p>
              <a href={`mailto:${c.email}`} className="block font-lato text-[15px] lg:text-[17px] text-ebony-clay leading-[30px] hover:text-crimson">
                {c.email}
              </a>
              <a href={`tel:${c.phone.replace(/\s/g, '')}`} className={`block font-lato text-[15px] lg:text-[17px] text-ebony-clay leading-[30px] hover:text-crimson ${phoneBold ? 'font-bold' : ''}`}>
                {c.phone}
              </a>
            </div>
          )
          return (
            <div className="max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-x-16 lg:gap-y-6">
              {/* LEFT ROW 1 — general + blue box */}
              <div className="flex justify-center lg:justify-start lg:col-start-1 lg:row-start-1">
                <div className="w-full max-w-[400px]">
                  {general && <div className="mb-10 lg:mb-12">{entry(general, true)}</div>}
                  <div className="border border-royal-blue py-10 lg:py-14 px-6 space-y-9 lg:space-y-10">
                    {rest.map((c) => entry(c))}
                  </div>
                </div>
              </div>

              {/* RIGHT — form card spans both left rows so its bottom aligns with the box's bottom.
                 h-full stretches the card to the height of the matching left row (general + box). */}
              <div className="bg-concrete rounded-[20px] lg:rounded-[40px] p-5 lg:p-[40px] w-full max-w-[600px] mx-auto lg:mx-0 lg:justify-self-end lg:col-start-2 lg:row-start-1 lg:h-full">
                <p className="font-lato text-[18px] lg:text-[20px] font-bold text-ebony-clay leading-[30px] mb-6">
                  I would like to know more about the services
                </p>
                <InquiryForm />
              </div>

              {/* LEFT ROW 2 — phone / WhatsApp icons (below the box, 400px-wide stack to match above) */}
              <div className="flex justify-center lg:justify-start lg:col-start-1 lg:row-start-2">
                <div className="w-full max-w-[400px]">
                  <div className="flex items-center justify-center gap-6">
                    <a href="tel:+94117778777" aria-label="Call us">
                      <img src={phoneIcon} alt="Call us" className="w-10 h-10" loading="lazy" />
                    </a>
                    <a href="https://wa.me/94777800606" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp">
                      <img src={whatsappIcon} alt="WhatsApp" className="w-10 h-10" loading="lazy" />
                    </a>
                  </div>
                </div>
              </div>
        ...
        {/* OUR PRESENCE — text left (OUR PRESENCE heading + OFFICE block + WE ARE SOCIAL), map right */}
        <div className="max-w-[1200px] mx-auto mt-16 lg:mt-24 flex flex-col lg:flex-row gap-10 lg:gap-16 lg:items-end">
          <div className="w-full lg:w-[380px] lg:flex-shrink-0">
            <p className="font-lato text-[20px] lg:text-[26px] font-normal text-crimson tracking-[0.3em] lg:tracking-[0.4em] uppercase mb-8 lg:mb-10">
              OUR PRESENCE
            </p>

            <p className="font-lato text-[18px] lg:text-[20px] font-bold text-royal-blue uppercase mb-3">
              OFFICE
            </p>
            <p className="font-lato text-[16px] lg:text-[18px] text-ebony-clay leading-[32px]">
              199/58,<br />Rajagiriya Road,<br />Rajagiriya.
            </p>

            <p className="social-heading uppercase mt-8 lg:mt-10 mb-5">WE ARE SOCIAL</p>
            <div className="flex flex-wrap items-center gap-x-4 gap-y-3">
              <a href="https://www.facebook.com/bimsararealestate/" target="_blank" rel="noopener noreferrer"><img src={fbIcon} alt="Facebook" className="w-12 h-12 object-contain block" loading="lazy" /></a>
              <a href="https://g.page/r/CcszCwgyi0e4EAE" target="_blank" rel="noopener noreferrer"><img src={googleIcon} alt="Google" className="w-12 h-12 object-contain block" loading="lazy" /></a>
              <a href="https://www.linkedin.com/company/safetynet-private-limited" target="_blank" rel="noopener noreferrer"><img src={linkedinIcon} alt="LinkedIn" className="w-12 h-12 object-contain block" loading="lazy" /></a>
              <a href="https://www.instagram.com/bimsara.realestate/" target="_blank" rel="noopener noreferrer"><img src={instagramIcon} alt="Instagram" className="w-12 h-12 object-contain block" loading="lazy" /></a>
              <a href="https://www.youtube.com/channel/UC7qFW8s7CdI1GeDChAY5nTw/featured" target="_blank" rel="noopener noreferrer"><img src={youtubeIcon} alt="YouTube" className="w-12 h-12 object-contain block" loading="lazy" /></a>
            </div>
          </div>
          ...
          <div className="flex-1 w-full">
            <iframe
              ...
              className="w-full h-[240px] lg:h-[340px] rounded-[8px] border-0"
              allowFullScreen
              loading="lazy"
            />
          </div>
